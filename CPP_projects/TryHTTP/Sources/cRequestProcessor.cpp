...
{
	int nParamsStart = ParseFirstLine(pchMessageBuffer, NSize, requestData);
	if ( nParamsStart <= 0)
	{
		return;
	}

	int nRetVal = ParseParams(static_cast<const char *>(&pchMessageBuffer[nParamsStart]),
								static_cast<const int &>(NSize - nParamsStart - 1),
								requestData);
...
int cRequestProcessor::ParseFirstLine(const char * pchMessageBuffer, const int & NSize, REQUEST_DATA & requestData) const
{
	int nStart = 0;
	int nEnd = 0;
